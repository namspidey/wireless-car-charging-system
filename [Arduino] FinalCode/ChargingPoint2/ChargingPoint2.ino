...
float readCurrent(int current_pin) {
  ...
  float current = (averageVoltage - 2.54) / 0.100;
...
String formatWithCommas(long number) {
  ...
  while (insertPosition > 0) {
    formatted = formatted.substring(0, insertPosition) + "," + formatted.substring(insertPosition);
    insertPosition -= 3;
  }
...
void loop() {
  ...
  if (detectCar) {
    ...
    if (receivedData == "0") {
      ...
      } else if (isCharging && point_current < 0.35) {
        system_ready();
        isCharging = false; 
        ...
        if (vehicleLicensePlate != "") {
          ...
          if (receivedData == "1") {
            ...
            if (separatorIndex != -1) {
              ...
              long balance = balanceStr.toInt();  
             
              String formattedBalance = formatWithCommas(balance);

              lcd.clear();
              lcd.setCursor(0, 0);
              lcd.print("Welcome " + name + "!");
              lcd.setCursor(0, 1);
              lcd.print("$: " + formattedBalance + " VND");
